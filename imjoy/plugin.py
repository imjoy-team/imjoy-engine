...
def resumePluginSession(eng, pid, session_id, plugin_signature):
    ...
    plugin_sessions = eng.ws_conn.data.plugin_sessions
    plugin_signatures = eng.ws_conn.data.plugin_signatures
    if pid in plugins:
...
def addClientSession(eng, session_id, client_id, sid, base_url, workspace):
    """Add client session."""
    logger = eng.logger
    clients = eng.ws_conn.data.clients
...
def addPlugin(eng, plugin_info, sid=None):
    ...
    plugins = eng.ws_conn.data.plugins
    plugin_sessions = eng.ws_conn.data.plugin_sessions
    plugin_sids = eng.ws_conn.data.plugin_sids
    plugin_signatures = eng.ws_conn.data.plugin_signatures
    pid = plugin_info["id"]
...
async def killAllPlugins(eng, ssid):
    """Kill all plugins."""
    logger = eng.logger
    on_kill_plugin = eng.ws_conn.sio.handlers[NAME_SPACE]["kill_plugin"]
    plugin_sids = eng.ws_conn.data.plugin_sids
    tasks = []
