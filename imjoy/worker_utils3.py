...
class FuturePromise(Promise, asyncio.Future):
    ...
    def resolve(self, result):
        ...
        if self._resolve_handler or self._finally_handler:
            ...
            self.loop.call_soon_threadsafe(self.set_result, result)
    ...
    def reject(self, error):
        ...
        if self._catch_handler or self._finally_handler:
            ...
            if error:
                self.loop.call_soon_threadsafe(self.set_exception, Exception(str(error)))
            else:
                self.loop.call_soon_threadsafe(self.set_exception, Exception())
